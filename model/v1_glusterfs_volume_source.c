...
v1_glusterfs_volume_source_t *v1_glusterfs_volume_source_create(
	...
	v1_glusterfs_volume_source_local_var->readOnly = readOnly;
...
cJSON *v1_glusterfs_volume_source_convertToJSON(v1_glusterfs_volume_source_t *v1_glusterfs_volume_source) {
	...
	// v1_glusterfs_volume_source->readOnly
    if(v1_glusterfs_volume_source->readOnly) { 
    if(cJSON_AddBoolToObject(item, "readOnly", v1_glusterfs_volume_source->readOnly) == NULL) {
    goto fail; //Bool
    }
     } 
...
v1_glusterfs_volume_source_t *v1_glusterfs_volume_source_parseFromJSON(cJSON *v1_glusterfs_volume_sourceJSON){
    ...
    // v1_glusterfs_volume_source->readOnly
    cJSON *readOnly = cJSON_GetObjectItemCaseSensitive(v1_glusterfs_volume_sourceJSON, "readOnly");
    if (readOnly) { 

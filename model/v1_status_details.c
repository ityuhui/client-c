...
v1_status_details_t *v1_status_details_create(
	...
	v1_status_details_local_var->retryAfterSeconds = retryAfterSeconds;
...
cJSON *v1_status_details_convertToJSON(v1_status_details_t *v1_status_details) {
	...
	// v1_status_details->retryAfterSeconds
    if(v1_status_details->retryAfterSeconds) { 
    if(cJSON_AddNumberToObject(item, "retryAfterSeconds", v1_status_details->retryAfterSeconds) == NULL) {
    goto fail; //Numeric
    }
     } 
...
v1_status_details_t *v1_status_details_parseFromJSON(cJSON *v1_status_detailsJSON){
    ...
    // v1_status_details->retryAfterSeconds
    cJSON *retryAfterSeconds = cJSON_GetObjectItemCaseSensitive(v1_status_detailsJSON, "retryAfterSeconds");
    if (retryAfterSeconds) { 
    if(!cJSON_IsNumber(retryAfterSeconds))
    {
    goto end; //Numeric
    }
    }
    ...
    v1_status_details_local_var = v1_status_details_create (
        ...
        retryAfterSeconds ? retryAfterSeconds->valuedouble : 0,

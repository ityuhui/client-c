...
v1_cluster_role_t *v1_cluster_role_create(
    v1_aggregation_rule_t *aggregationRule,
    char *apiVersion,
    char *kind,
	...
	v1_cluster_role_local_var->aggregationRule = aggregationRule;
	v1_cluster_role_local_var->apiVersion = apiVersion;
...
void v1_cluster_role_free(v1_cluster_role_t *v1_cluster_role) {
    ...
    v1_aggregation_rule_free(v1_cluster_role->aggregationRule);
    free(v1_cluster_role->apiVersion);
...
cJSON *v1_cluster_role_convertToJSON(v1_cluster_role_t *v1_cluster_role) {
	...
	// v1_cluster_role->aggregationRule
    if(v1_cluster_role->aggregationRule) { 
    cJSON *aggregationRule_local_JSON = v1_aggregation_rule_convertToJSON(v1_cluster_role->aggregationRule);
	...
	// v1_cluster_role->apiVersion
    if(v1_cluster_role->apiVersion) { 
    if(cJSON_AddStringToObject(item, "apiVersion", v1_cluster_role->apiVersion) == NULL) {
    goto fail; //String
    }
     } 
...
v1_cluster_role_t *v1_cluster_role_parseFromJSON(cJSON *v1_cluster_roleJSON){
    ...
    // v1_cluster_role->aggregationRule
    cJSON *aggregationRule = cJSON_GetObjectItemCaseSensitive(v1_cluster_roleJSON, "aggregationRule");
    v1_aggregation_rule_t *aggregationRule_local_nonprim = NULL;
    if (aggregationRule) { 
    aggregationRule_local_nonprim = v1_aggregation_rule_parseFromJSON(aggregationRule); //nonprimitive
    }

    // v1_cluster_role->apiVersion
    cJSON *apiVersion = cJSON_GetObjectItemCaseSensitive(v1_cluster_roleJSON, "apiVersion");
    if (apiVersion) { 

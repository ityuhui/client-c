...
v1_pod_t *v1_pod_create(
	...
	v1_pod_local_var->apiVersion = apiVersion;
...
cJSON *v1_pod_convertToJSON(v1_pod_t *v1_pod) {
	...
	// v1_pod->apiVersion
    if(v1_pod->apiVersion) { 
    if(cJSON_AddStringToObject(item, "apiVersion", v1_pod->apiVersion) == NULL) {
    goto fail; //String
    }
     } 
...
v1_pod_t *v1_pod_parseFromJSON(cJSON *v1_podJSON){
    ...
    // v1_pod->apiVersion
    cJSON *apiVersion = cJSON_GetObjectItemCaseSensitive(v1_podJSON, "apiVersion");
    if (apiVersion) { 

...
v1_rbd_volume_source_t *v1_rbd_volume_source_create(
	...
	v1_rbd_volume_source_local_var->fsType = fsType;
	...
	v1_rbd_volume_source_local_var->readOnly = readOnly;
	v1_rbd_volume_source_local_var->secretRef = secretRef;
...
void v1_rbd_volume_source_free(v1_rbd_volume_source_t *v1_rbd_volume_source) {
    ...
    free(v1_rbd_volume_source->fsType);
    ...
    v1_local_object_reference_free(v1_rbd_volume_source->secretRef);
    free(v1_rbd_volume_source->user);
	free(v1_rbd_volume_source);
}

cJSON *v1_rbd_volume_source_convertToJSON(v1_rbd_volume_source_t *v1_rbd_volume_source) {
	cJSON *item = cJSON_CreateObject();

	// v1_rbd_volume_source->fsType
    if(v1_rbd_volume_source->fsType) { 
    if(cJSON_AddStringToObject(item, "fsType", v1_rbd_volume_source->fsType) == NULL) {
    goto fail; //String
    }
     } 
	...
	// v1_rbd_volume_source->readOnly
    if(v1_rbd_volume_source->readOnly) { 
    if(cJSON_AddBoolToObject(item, "readOnly", v1_rbd_volume_source->readOnly) == NULL) {
    goto fail; //Bool
    }
     } 


	// v1_rbd_volume_source->secretRef
    if(v1_rbd_volume_source->secretRef) { 
    cJSON *secretRef_local_JSON = v1_local_object_reference_convertToJSON(v1_rbd_volume_source->secretRef);
...
v1_rbd_volume_source_t *v1_rbd_volume_source_parseFromJSON(cJSON *v1_rbd_volume_sourceJSON){

    v1_rbd_volume_source_t *v1_rbd_volume_source_local_var = NULL;

    // v1_rbd_volume_source->fsType
    cJSON *fsType = cJSON_GetObjectItemCaseSensitive(v1_rbd_volume_sourceJSON, "fsType");
    if (fsType) { 
    ...
    // v1_rbd_volume_source->image
    cJSON *image = cJSON_GetObjectItemCaseSensitive(v1_rbd_volume_sourceJSON, "image");
    if (!image) {
        goto end;
    }
    ...
    // v1_rbd_volume_source->keyring
    cJSON *keyring = cJSON_GetObjectItemCaseSensitive(v1_rbd_volume_sourceJSON, "keyring");
    if (keyring) { 
    ...
    // v1_rbd_volume_source->monitors
    cJSON *monitors = cJSON_GetObjectItemCaseSensitive(v1_rbd_volume_sourceJSON, "monitors");
    if (!monitors) {
        goto end;
    }
    ...
    // v1_rbd_volume_source->pool
    cJSON *pool = cJSON_GetObjectItemCaseSensitive(v1_rbd_volume_sourceJSON, "pool");
    if (pool) { 
    ...
    // v1_rbd_volume_source->readOnly
    cJSON *readOnly = cJSON_GetObjectItemCaseSensitive(v1_rbd_volume_sourceJSON, "readOnly");
    if (readOnly) { 
    ...
    // v1_rbd_volume_source->secretRef
    cJSON *secretRef = cJSON_GetObjectItemCaseSensitive(v1_rbd_volume_sourceJSON, "secretRef");
    ...
    // v1_rbd_volume_source->user
    cJSON *user = cJSON_GetObjectItemCaseSensitive(v1_rbd_volume_sourceJSON, "user");
    if (user) { 

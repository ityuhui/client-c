...
v1_status_t *v1_status_create(
	...
	v1_status_local_var->apiVersion = apiVersion;
...
cJSON *v1_status_convertToJSON(v1_status_t *v1_status) {
	...
	// v1_status->apiVersion
    if(v1_status->apiVersion) { 
    if(cJSON_AddStringToObject(item, "apiVersion", v1_status->apiVersion) == NULL) {
    goto fail; //String
    }
     } 
...
v1_status_t *v1_status_parseFromJSON(cJSON *v1_statusJSON){
    ...
    // v1_status->apiVersion
    cJSON *apiVersion = cJSON_GetObjectItemCaseSensitive(v1_statusJSON, "apiVersion");
    if (apiVersion) { 
